...
5. En **Clusters → Connect → Drivers → Node.js**, copia la URI:
   ```
   mongodb+srv://Fluenta_admin:<password>@cluster0.xxxxx.mongodb.net/Fluenta?retryWrites=true&w=majority
   ```
...
2. Conecta tu repo de GitHub: `SebastianCreator/Fluenta`
...
4. Añade variables de entorno:
   ...
   CLIENT_URL=https://Fluenta.vercel.app
   ```
...
2. Importa `SebastianCreator/Fluenta`
3. Vercel detectará automáticamente el framework Vite (gracias a `vercel.json` raíz)
4. Configura variable de entorno:
   - `VITE_API_URL`: URL de tu backend en Render (ej: `https://Fluenta-api.onrender.com/api`)
...
1. Una vez deployado el backend en Render, copia su URL (ej: `https://Fluenta-api.onrender.com`)
2. En Vercel, ve a **Settings → Environment Variables**
3. Añade/actualiza:
   ```
   VITE_API_URL=https://Fluenta-api.onrender.com/api
   ```
4. Re-deploy el frontend para aplicar cambios

## 5️⃣ Verificación

- [ ] Backend health check: `https://Fluenta-api.onrender.com/health`
